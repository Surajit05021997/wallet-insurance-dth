<template>
  <div>
    <add-family-member
      @family-member-added="fetchFamilyMembersList()"
    ></add-family-member>
    <div class="d-flex mb-5 mt-5">
      ...
      <button
        type="button"
        class="btn btn-outline-success btn-text-1_5rem rounded-3"
    ...
    <div v-if="familyMembersList">
      <div v-if="familyMembersList.length > 0">
        <table class="table table-hover text-1_7rem">
          <thead>
            <tr>
              <th v-for="(field, index) in familyMembersFields" :key="index">
                {{ field }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(familyMember, index) in familyMembersList" :key="index">
              ...
              <td>
                <span>
                  <input
                    type="checkbox"
                    class="btn-check"
                    :id="familyMember.familyMemberMobile"
                    autocomplete="off"
                    :value="familyMember.isAuthorised"
                    @change="changeAuthorisation(familyMember)"
                  />
                  <label
                    class="btn btn-outline-primary"
                    :for="familyMember.familyMemberMobile"
                  >
              ...
              <td>
                <button
                  @click="removeFamilyMember(familyMember.id)"
                  type="button"
      ...
      <div v-if="familyMembersList.length === 0">
        ...
        <h4>
          You can authorise family members for some actions to be done on your
          behalf.
        </h4>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async changeAuthorisation(familyMember) {
      ...
      const editedFamilyMemberStatusText = await updateFamilyMember(
        familyMember.id,
        editedFamilyMember
      );
